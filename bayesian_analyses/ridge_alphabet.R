...
ridge_alphabet<-function (dat, range_file=NULL, xlim, labx, wi, hi) 
{
  ...
  title.size <- 30
  x.axis.size <- 25
  p <- ggplot(dataset, aes(x = x.values, y = y.values, fill = distrib.fill, group = group)) + 
    ...
    theme(plot.title = element_text(vjust = -0.5, size = title.size), axis.text.y.left = element_text(size = ROI.label.size),
                                                                          plot.margin = margin(t = 1, r = 3.5, b = 1, l = 1, unit = "cm"),
                                                                          axis.text.y.right = ggtext::element_markdown(size = P.label.size, lineheight = 1.2), 
  ...
  g <- ggplotGrob(p)
  left_axis_width <- convertWidth(sum(g$widths[g$layout$name == "axis-l"]), "in", valueOnly = TRUE)
  right_axis_width <- convertWidth(sum(g$widths[g$layout$name == "axis-r"]), "in", valueOnly = TRUE)
  plot_panel_width <- wi
  total_width <- left_axis_width + plot_panel_width + right_axis_width + 2 
  
  ggsave(file = paste0(labx, "_alphabet.jpg"), plot = p, width = total_width, height = hi, dpi = 500)
}
